...
function DollieMantoTracker() {
  const [dollieId, setDollieId] = useState('')
  const [dollieData, setDollieData] = useState(null)
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await window.api.updateDollie(
        dollieId,
        isDamaged ? 'yes' : 'no',
        observationDmg || '',
        screwsCountNumber,
      ...
      if (response.success) {
        setDollieId('')
        setSuccessMessage('Dollie actualizado exitosamente.')
        setTimeout(() => setSuccessMessage(''), 3000)
      } else {
        setErrorMessage(response.message || 'Error al actualizar el dollie.')
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await window.api.searchDollyById(dollieId)
        if (response.success) {
          setDollieData(true)
        } else if (dollieId !== '') {
          setErrorMessage('No se encontro un dollie con ese ID.')
          setTimeout(() => setErrorMessage(''), 3000)
          setDollieData(false)
        } else {
          setDollieData(false)
    ...
    >
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        <form onSubmit={handleFormSubmit}>
          <div className="mb-4">
            <label htmlFor="dollieId" className="block text-sm font-medium text-gray-700">
              ID del Dollie:
            </label>
            <input
              type="text"
              id="dollieId"
              value={dollieId}
              onChange={(e) => setDollieId(e.target.value)}
...
export default DollieMantoTracker
